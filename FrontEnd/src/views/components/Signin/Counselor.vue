<template>
  <div>
    ...
    <div class="col-lg-12 row m-0">
      ...
      <input type="checkbox" id="자폐증" value="1" v-model="datas.expertise_no">
      <label for="자폐증">자폐증</label>
      <label for="file" class="col col-lg-2 mt-3 "></label>
      <input type="checkbox" id="아스퍼거" value="2" v-model="datas.expertise_no">
      <label for="아스퍼거 증후군">아스퍼거 증후군</label>
      <label for="file" class="col col-lg-2 mt-3 "></label>
      <input type="checkbox" id="전반적 발달 장애" value="3" v-model="datas.expertise_no">
      <label for="전반적 발달 장애">전반적 발달 장애</label>
      <label for="file" class="col col-lg-2 mt-3 "></label>
      <input type="checkbox" id="레트 증후군" value="4" v-model="datas.expertise_no">
      <label for="레트 증후군">레트 증후군</label>
      <label for="file" class="col col-lg-2 mt-3 "></label>
      <input type="checkbox" id="아동기 붕괴성 장애" value="5" v-model="datas.expertise_no">
    ...
    <div class="col-lg-12 row m-0">
      ...
      <base-button type="secondary" class="ni ni-fat-add p-1" @click="학력추가 = true"></base-button>
      <div class="col-lg-12 row teach py-3 m-0" v-if="학력추가 == true">
        ...
        <input type="date" class="col-lg-3 form-control mt-2" id="year" v-model="admission"/>
        <div class="col-lg-1"></div>
        <label for="year" class="col col-lg-2 mt-3 ">졸업</label>
        <input type="date" class="col-lg-3 form-control mt-2" id="year" v-model="graduation" />
    ...
    <div v-for="(academicCareer, i) in datas.academicCareers" :key="i">
      <div class="col-lg-12 row teach py-3 m-0">
        <label for="school" class="col col-lg-2 mt-2">학교명: {{ academicCareer[0] }}</label>
        <div class="col-lg-1"></div>
        <label for="major" class="col col-lg-2 mt-2">전공: {{ academicCareer[1] }}</label>
        <label for="inyear" class="col col-lg-2 mt-3 ">입학일: {{ academicCareer[2] }}</label>
        <div class="col-lg-1"></div>
        <label for="year" class="col col-lg-2 mt-3 ">졸업일: {{ academicCareer[3] }}</label>
        <p>
          <button>삭제</button>
        </p>
      </div>
    ...
    <div class="col-lg-12 row m-0">
      ...
      <base-button type="secondary" class="ni ni-fat-add p-1" @click="경력추가 = true"></base-button>
    ...
    <div v-for="(career, i) in datas.careers" :key="i">
      <div class="col-lg-12 row teach py-3 m-0">
        <label for="school" class="col col-lg-2 mt-2">회사명: {{ career[0] }}</label>
        <div class="col-lg-1"></div>
        <label for="major" class="col col-lg-2 mt-2">직급: {{ career[1] }}</label>
        <label for="inyear" class="col col-lg-2 mt-3 ">재직기간: {{ career[2].range }}</label>
        <div class="col-lg-1"></div>
        <label for="year" class="col col-lg-2 mt-3 ">담당업무: {{ career[3] }}</label>
        <p>
          <button>삭제</button>
        </p>
      </div>
    ...
    <div class="col-lg-12 row m-0">
      ...
      <base-button type="secondary" class="ni ni-fat-add p-1" @click="자격증추가 = true"></base-button>
      <div class="col-lg-12 row teach py-3 m-0" v-if="자격증추가 == true">
        ...
        <input type="date" class="col-lg-3 form-control mt-2" id="cer_get" v-model="acquisition_date"/>
    ...
    <div v-for="(licence, i) in datas.licences" :key="i">
      <div class="col-lg-12 row teach py-3 m-0">
        <label for="school" class="col col-lg-2 mt-2">자격증명: {{ licence[0] }}</label>
        <div class="col-lg-1"></div>
        <label for="major" class="col col-lg-2 mt-2">발행처/기관: {{ licence[1] }}</label>
        <label for="inyear" class="col col-lg-2 mt-3 ">취득일: {{ licence[2] }}</label>
        <div class="col-lg-1"></div>
        <label for="year" class="col col-lg-2 mt-3 ">첨부파일: {{ licence[3] }}</label>
        <p>
          <button>삭제</button>
        </p>
      </div>
  ...
  methods: {
    addAcadimicCareer() {
      if (this.university && this.major && this.admission && this.graduation) {
        this.datas.academicCareers.push([this.university, this.major, this.admission, this.graduation])
        this.university = '',
        this.major = '',
        this.admission = '',
        this.graduation = '',
    ...
    addCareer() {
      if (this.company && this.position && this.dates && this.part) {
        this.datas.careers.push([this.company, this.position, this.dates, this.part])
        this.company = '',
        this.position = '',
        this.dates = '',
        this.part = '',
    ...
    addlicence() {
      if (this.licence_name && this.licence_publication && this.acquisition_date) {
        this.datas.licences.push([this.licence_name, this.licence_publication, this.acquisition_date, this.licence_file])
        this.licence_name = '',
